...
class BenchClampDatasetConfig(ClampDataConfig):
    ...
    def setup_data(self) -> Tuple[List[FullDatum], List[FullDatum], List[FullDatum]]:
        ...
        if self.dataset_name == "CalFlowFindEvent":
            train_data_file = "./data/20230222-find_event.train.jsonl"
            dev_data_file = "./data/20230222-find_event.valid.jsonl" 
            test_data_file = "./data/20230222-find_event.valid.jsonl" 

        if self.dataset_name == "CalFlowFindEvent-Subset":
            train_data_file = "./data/20230222-find_event.train.jsonl"
            dev_data_file = "./data/20230222-find_event.subset.valid.jsonl"
            test_data_file = "./data/20230222-find_event.subset.valid.jsonl"

        if self.dataset_name == "CalFlowFindEventRevise":
            train_data_file = "./data/20230222-find_event_revise.train.proportional_split.jsonl"
            dev_data_file = "./data/20230222-find_event_revise.valid.proportional_split.text-davinci-003.8_examples.jsonl"
            test_data_file = "./data/20230222-find_event_revise.valid.proportional_split.text-davinci-003.8_examples.jsonl"

        
        if self.dataset_name == "CalFlowFindEventTrainReviseDev":
            train_data_file = "./data/20230222-find_event.train.jsonl"
...
class BenchClampDataset(str, Enum):
    ...
    CalFlowFindEventSubset = "CalFlowFindEvent-Subset"
    CalFlowFindEventRevise = "CalFlowFindEventRevise"
    CalFlowFindEventTrainReviseDev = "CalFlowFindEventTrainReviseDev"
    CalFlowFindEventReviseEditFragment = "CalFlowFindEventReviseEditFragment"
...
BENCHCLAMP_DATA_CONFIGS: List[ClampDataConfig] = (
    ...
    [
        ...
        [
        BenchClampDatasetConfig(
            data_id=f"calflowfindeventsubset_{input_sequence_creator_name}_{split_name}",
            split_name=split_name,
            domain=None,
            dataset_name=BenchClampDataset.CalFlowFindEventSubset.value,
            input_sequence_creator=input_sequence_creator,
        )
        for input_sequence_creator_name, input_sequence_creator, split_names in [
            ("no_context", IdentitySequenceCreator(), BENCHCLAMP_SPLIT_NAMES),
    ...
    [
        BenchClampDatasetConfig(
            data_id=f"calflowfindeventreviseeditfragment_{input_sequence_creator_name}_{split_name}",
            split_name=split_name,
            domain=None,
            dataset_name=BenchClampDataset.CalFlowFindEventReviseEditFragment.value,
            input_sequence_creator=input_sequence_creator,
        )
        for input_sequence_creator_name, input_sequence_creator, split_names in [
            ("no_context", IdentitySequenceCreator(), BENCHCLAMP_SPLIT_NAMES),
    ...
    [
        BenchClampDatasetConfig(
            data_id=f"calflowfindevent_train_revise_dev_{input_sequence_creator_name}_{split_name}",
            split_name=split_name,
            domain=None,
            dataset_name=BenchClampDataset.CalFlowFindEventTrainReviseDev.value,
            input_sequence_creator=input_sequence_creator,
        )
        for input_sequence_creator_name, input_sequence_creator, split_names in [
            ("no_context", IdentitySequenceCreator(), BENCHCLAMP_SPLIT_NAMES),
